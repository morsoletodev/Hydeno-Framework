...
from ..config import get_link_config, get_global_config
...
def train_splink():
    ...
    try:
        linker = create_linker(db_api)
        logger.info("Linker object created")

        link_c = get_link_config()
        ...
        linker.training.estimate_u_using_random_sampling(
            max_pairs=link_c.max_pairs, seed=link_c.seed
        )
        ...
        linker.misc.save_model_to_json(
            get_global_config().f_splink_model, overwrite=True
        )
